...
#### 6. Пережатие существующего файла
```
...
- Если оригинала нет, использует текущую сжатую версию как источник.
- Создает новую запись в БД и **удаляет старую** (soft delete) при успехе.
- Автоматическая дедупликация: если такой файл уже есть, вернет его.
