...
### 4. Улучшение работы с `metadata` и `optimizationParams`
...
### 1. Linter Errors
Команда `npm run lint` сообщает о ~330 проблемах. Основные категории:
...
Сервис `FilesService` содержит около 670 строк кода. Метод `uploadFileStream` выполняет слишком много задач: создание записи в БД, загрузка в S3, хеширование, дедупликация, копирование.
...
3. Униформация типов Enums между Prisma и приложением.
